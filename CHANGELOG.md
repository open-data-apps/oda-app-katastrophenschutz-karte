...
## 1.24.0 - 2026-08-25
- **FIX:** `apiurls.anlaufstellen`-Default auf `../assets/daten-beispiel.csv` korrigiert (fehlendes `../`); gebündelte Demo-Ressource liegt damit wie dokumentiert im Liefer-ZIP.
...
## 1.22.0 - 2026-08-21
- **CHG:** Skalares `apiurl` durch das Array-Feld `apiurls` ersetzt (`typ: "array"`, Eintrag `anlaufstellen`). Neuer Standard portfolioweit; `apiurl` entfällt. `app.js` liest die Datenquelle jetzt über `getOdasApiUrl(configdata, "anlaufstellen")`.
